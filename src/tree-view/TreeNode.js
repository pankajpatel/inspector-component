...
const defaultNodeRenderer = ({ depth, name, data, isNonenumerable }) =>
  depth === 0
    ? `<object-root-label name='${
        name !== undefined ? name : ""
      }' data='${data}' ></object-root-label>`
    : `<object-label name='${
        name !== undefined ? name : ""
      }' data='${data}' isNonenumerable='${isNonenumerable}' ></object-label>`;
          ...
          toCss(
            Object.assign(
              ...
              newValue === true || newValue === "true"
                ? this.styles.expanded
                : this.styles.collapsed
            )
          )
        );
      }
...
class TreeNode extends HTMLElement {
  connectedCallback() {
    const nodeRenderer =
      defaultNodeRenderer || (({ name }) => `<span>${name}</span>`);
    ...
    this.shouldShowArrow =
      this.getAttribute("should-show-arrow") == "false" ? false : true;
    this.shouldShowPlaceholder =
      this.getAttribute("should-show-placeholder") == "true" ? true : false;
    ...
    this.innerHTML = `
      <li aria-expanded='${this.expanded}' role="treeitem" style='${toCss(
      styles.treeNodeBase
    )}' title='${this.title}'>
        <div style='${toCss(styles.treeNodePreviewContainer)}' path='${
      this.path
    }' class="clickableNode">
          ${
            this.expandable
              ? `<tree-arrow expanded='${
                  ...
                  styles.treeNodeArrow
                )}'></tree-arrow>`
              : this.shouldShowPlaceholder
              ? `<span style='${toCss(
                  styles.treeNodePlaceholder
                )}'>&nbsp;</span>`
              : ""
          }
          ${renderedNode}
