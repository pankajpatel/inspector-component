...
const defaultNodeRenderer = ({ depth, name, data, isNonenumerable }) => depth === 0 ? `<object-root-label name='${name !== undefined ? name : ''}' data='${data}' ></object-root-label>` : `<object-label name='${name !== undefined ? name : ''}' data='${data}' isNonenumerable='${isNonenumerable}' ></object-label>`;
...
class Arrow extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this.innerHTML = `<span style='${toCss(Object.assign({}, this.styles.base, expanded === true ? this.styles.expanded : this.styles.collapsed))}'>▶</span>`;
  }
}
